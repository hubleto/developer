# Lesson 1: Introduction to Hubleto

<i class="fas fa-medal mr-2"></i> Developer Certification Level 1

## Introduction

In this lesson, the student will learn the basics about Hubleto ecosystem. The student will also be able to install Hubleto to his local development environment.

## Study material

| Resource                                                                                                                      | Description                                                                                             |
| ----------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| [About Hubleto](../../about)                                                                                                  | Introduction to Hubleto                                                                                 |
| [Introduction to Hubleto](../../downloads/webinar-introduction-to-hubleto/Introduction%20to%20Hubleto.pdf)                    | Presentation                                                                                            |
| [How to setup your development environment](../../downloads/webinar-introduction-to-hubleto/How%20to%20setup%20dev%20env.pdf) | Presentation                                                                                            |
| [Overview of Hubleto apps](../../downloads/webinar-introduction-to-hubleto/Overview%20of%20Hubleto%20apps.pdf)                | Presentation                                                                                            |
| [Install Hubleto](../../installation)                                                                                         | Step-by-step guide to install Hubleto. For this lesson you will need only **production-ready** version. |
| [Install configuration .yaml file](../../installation/init-config)                                                            | Description of `.yaml` file for storing initial configuration of your Hubleto project.                  |
| [`php hubleto` CLI agent](../../cli-agent)                                                                                    | Introduction to CLI agent.                                                                              |
| [Sample `CarRental` app](https://github.com/mrgopes/hubleto-car-rental)                                                       | Source code of a sample CarRental app.                                                                  |

## Videos

<iframe width="373" height="210" src="https://www.youtube.com/embed/9agAiqGtqmI?> si=7R88l7M2eTpfIhcE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>

## Do you have any question?

Do you have any question or comment? Leave us a message in the community portal.

<a class="btn" href="https://community.hubleto.eu/d/34-qa-developer-certification-level-1-lesson-1"><span class="text">Go to community.hubleto.eu</span></a>

If you are new here, learn more about [developer certification programme](../../certification).