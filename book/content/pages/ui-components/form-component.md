...
## Accesing the data of an entry

In the Form component you can access the data of an entry with `this.state.record`. Most of the Form components in Ceremony CRM saves record data into another variable `R`, which is used throughout the forms for accesing data or updating the record state with it.
...
Ceremony CRM uses a wrapper to create basic input elements. Creating an input wrapper with the name of the column in the model will automaticly create an input element that corresponds to the input type described in the column.
...
### Form Input and Input Componenents

The other method of creating input elements is to use the [ADIOS input components](adios/input) wrapped in a `FormInput` component. This is a way to manualy create input elements **without the description of the column from the model**. You are creating an input element from scratch, therefore you need to describe what the data will show, create, update and/or delete.
...
- `onChange` is an event callback that will be called everytime something in the input changes
...
For the example below we used the API endpoint `customers/get-company-contacts` specified in Loader.php of the Customer module. We also added the `customEndpointParams` prop that will pass the ID of the company as a paramenter to the API endpoint, which will be then availible to use in the endpoint. Check out how to create an [API endpoint](../controller#Creating%20an%20API%20Controller).
...
After creating a Tag model for a specific model, you can crate a Tag input element using the ADIOS Tag component. This displays the tags names created in the Tag model and displays them in the input using the cross-table for the specified model and the Tag model.
...
- `uid` - generated UID
- `showHeader` - a boolean that shows/hiddes the header of the table with a title and buttons
- `showFooter` - a boolean that shows/hiddes the footer of the table
